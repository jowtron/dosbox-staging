...
private:
	...
	// Pause primitives for the renderer thread. The `Render()` loop
	// blocks on `pause_cv` while `is_paused` is set so the synth's
	// internal state doesn't advance during a DOSBox pause.
