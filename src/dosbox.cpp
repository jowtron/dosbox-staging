...
// Reset wall-clock accounting on resume so `increase_ticks` doesn't see a
// multi-second gap from before pause and try to "catch up" by running tens
// of seconds of emulation in a burst. PIC time is frozen across pause; only
// the wall-clock counters need this rebase.
//
...
// go through here -- it pauses audio via MIXER_LockMixerThread directly
// in mapper.cpp, which composes correctly with this path because both
// agree on mixer.mutex.
//
...
static void update_audio_pause_state()
{
...
void DOSBOX_RequestShutdown()
{
	...
	if (DOSBOX_IsPaused() || DOSBOX_IsPauseRequested()) {
