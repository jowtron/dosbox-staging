...
// Pause API. The emulator is either running or paused; pauses come
// from two sources -- the user (hotkey, HTTP API) and host-window
// inactivity. The FSM and its transition policy live in `dosbox.cpp`;
// here we expose only intent. User pauses survive auto signals, and
// auto-resume never touches a user pause; the corresponding request
// helpers encode that.
//
