...
## Pacing when SDL's clock is missing or wrong: `nosound` and fast-forward

The mental model above assumes SDL's steady real-time pull is present to pace
everything. Two modes break that assumption, and both are handled the same
way: **size each block from the emulated time that actually elapsed, instead of
assuming a fixed blocksize equals a fixed slice of real time.**

The emulator thread advances *emulated* time — the PIC clock, read off-thread
via `PIC_AtomicIndex()` — throttled to real time by the main loop. That is a
separate clock from SDL's. In both modes below the mixer takes the PIC delta
since its previous iteration (`actual_time`) and requests
`actual_time * frames_per_tick` frames, so the frames it produces — and feeds
to capture — always match the emulated time that passed. The two modes share
this one code path in `mixer_thread_loop()`.

### `nosound`: no consumer, so the mixer paces itself

With `nosound = on` there is no SDL device and no `final_output` consumer — the
single real clock is gone and there is no back-pressure to lean on. The mixer
thread paces itself instead, sleeping roughly one block's duration each
iteration (`SDL_DelayPrecise`, which busy-waits the tail end for precision).
The mixed output is discarded; only the **capture feed** still matters here.

A fixed sleep alone is not enough: the loop also does real work (mixing,
effects, feeding capture) and the sleep can overshoot, so each iteration takes
a little longer than a block's nominal duration. If the mixer still produced a
fixed blocksize per iteration it would lay down fewer frames than a second of
emulated time calls for, and recordings (e.g. MT-32 music) would play back
slightly too fast. Sizing the block from `actual_time` removes the drift — the
sleep then governs only cadence and CPU use, never the captured frame count.

`nosound` also snaps `playback_gain` straight to its target so the pause fade
gate still fires — see gotcha 3 below.

### Fast-forward: the emulator outruns the clock

Holding fast-forward (`Alt`+`F12`) runs the emulator far faster than real time,
so much more emulated time passes per mixer iteration than a blocksize covers.
The mixer requests the full `actual_time * frames_per_tick` frames and mixes
them, then **"chipmunk"-squashes** them down to a single blocksize by sample
skipping (`fast_forward_buffer`) before handing that to SDL — which still pulls
at real time, so live playback sounds sped up.

Capture is fed the **full, un-squashed** block from inside `mix_samples()`,
before the squash. So a recording made during fast-forward still plays at
normal speed; only live playback is compressed.
...
1. **Keep the belt full while paused.** If the mixer simply stopped feeding
   `final_output` during pause, SDL would drain it empty (and its own device
   buffer would underrun); on resume nothing would back-pressure the mixer, so
   it would sprint — producing blocks as fast as the CPU allows until the queue
   refilled, each one also pushed into the capture feed with no real time
   elapsed, stretching recordings by ~one bufferful per pause/resume cycle.
...
2. **Synth renderer ordering.** A software synth's renderer runs ahead of the
   ...
   samples into the next capture. Resume order doesn't matter: while parked
   the renderer stops its own `audio_frame_fifo`, so a mixer `BulkDequeue()`
   racing the wake gets a short read (silence-padded by the synth's
   `MixerCallback()`) instead of blocking on an empty fifo. See the ordering
   comments in `set_subsystems_paused()`.
