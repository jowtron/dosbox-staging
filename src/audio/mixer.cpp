...
// the capture queue; never zero-pads. mix_samples produces in ~blocksize
// bursts paced by SDL, while this callback is paced by emulator ticks --
// two independent clocks. The queue can be momentarily empty when the
// consumer wakes up before the producer (most visibly across a pause/resume
// edge); padding with silence would splice zero samples into the captured
// WAV / AVI audio track. The producer catches up on its next iteration and
// FIFO ordering keeps capture bit-identical to a continuously-running
// capture.
