...
struct MixerSettings {
	...
	// Fade-out/-in gain, ramped by `mixer_thread_loop()` and polled by
	// the PausePending FSM in dosbox.cpp (via `MIXER_GetPlaybackGain()`)
	// to know when the fade-out has reached zero.
	//
	...
	// there's no coordination hazard; the atomic exists purely to make
	// the FSM's read well-defined.
...
// Fades out the SDL `output_buffer` between
// full and muted over a few ms to mask the step discontinuity that would
// result in an audible click when a non-zero sample is followed by 0.0 when
...
static void mixer_thread_loop()
{
	...
	// `mixer.playback_gain` at the end of each iteration so the
	// PausePending FSM in dosbox.cpp can observe when the fade-out has
	// completed.
	float playback_gain = mixer.playback_gain.load(std::memory_order_relaxed);
	const float fade_step = compute_fade_step();
...
// samples). `MIDI_Pause()` releases the lock before `MIXER_Pause()`
// re-takes it, so the two lock/unlock cycles are sequential, not nested.
//
void MIXER_Pause()
{
	MIXER_LockMixerThread();
