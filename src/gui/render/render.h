...
struct Render {
	...
	// don't read the live, possibly mid-frame `scale.cache`), and as the
	// input for `RENDER_RescaleLastFrame()` after a pause-time recreate.
	struct {
		alignas(uint64_t)
		        std::array<uint32_t, ScalerMaxWidth * ScalerMaxHeight> cache = {};
		...
		// `sharp` and back) drops it to false. `RENDER_GetCurrentImage()`
		// gates on this so screenshots / video capture never pick up
		// a frame at the previous geometry.
		bool valid = false;
