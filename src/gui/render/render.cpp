...
// `render.last_complete_source` so screenshots / video capture and
// pause-time re-scale can read a clean, complete frame instead of the live
// `render.scale.cache` (which is mid-update during a scanout).
//
...
void RENDER_RescaleLastFrame()
{
	...
	if (latched_height != render_height &&
	    latched_height != render_height * 2 &&
